<?php

// autogenerated: Wed, 10 Sep 2014 18:08:53 +0200

namespace Sulu\Component\PhpcrDecorator\PHPCR\Query\QOM;


class AndConstraint implements \PHPCR\Query\QOM\AndInterface
{
    protected $component;
    protected $decoratorFactory;

    public function __construct(
        \PHPCR\Query\QOM\AndInterface $component,
        \Sulu\Component\PhpcrDecorator\DecoratorFactoryInterface $decoratorFactory
    )
    {
        $this->component = $component;
        $this->decoratorFactory = $decoratorFactory;
    }

    /**
     * {@inheritDoc}
     */
    public function getConstraint1()
    {
        return $this->decoratorFactory->decorate(__METHOD__, $this->component->getConstraint1());
    }

    /**
     * {@inheritDoc}
     */
    public function getConstraint2()
    {
        return $this->decoratorFactory->decorate(__METHOD__, $this->component->getConstraint2());
    }

}